...
/*var reqPg = function(reqSql, req, res, next){
...
/*  var resPg = reqPg("Select r.id_recipe, r.title_recipe, d.title_difficulty, t.title_type, r.time_recipe, r.popularity, r.peopleamount, o.title_origin, r.description, r.imgurl FROM public.recipe r, public.difficulty d, public.origin o, public.type t WHERE r.id_difficulty = d.id_difficulty AND r.id_origin = o.id_origin AND r.id_type = t.id_type;",req,res);
  console.log("test : " +  resPg);*/
  pg.connect(uripg  ,function(err, client, done) {
    if (err) throw err;
    client
...
var findRecipes = function(req, res){
  pg.connect(uripg  ,function(err, client, done) {
    if (err) throw err;
    client
      .query("SELECT r.id_recipe, r.title_recipe, r.time_recipe, r.description, d.title_difficulty, r.popularity, r.peopleamount, r.imgurl, t.title_type, o.title_origin, d.title_difficulty, f.title_food, c.quantity_containfood FROM public.recipe r, public.containfood c, public.food f, public.difficulty d, public.origin o, public.type t WHERE r.id_recipe = c.id_recipe AND c.id_food = f.id_food AND r.id_difficulty = d.id_difficulty AND r.id_origin = o.id_origin AND r.id_type = t.id_type;")
      .on('end', function(result) { done();
      var recipes = [];
      var title_recipe_Arr = [];
      var title_food_Arr = [];
      var quantity_containfood_Arr = [];

      result.rows.forEach(function(row){
        var recipe = {};
        if(title_recipe_Arr.indexOf(row.title_recipe) == -1){
          title_recipe_Arr.push(row.title_recipe);
          recipe.id_recipe = row.id_recipe;
          recipe.title_recipe = row.title_recipe;
          recipe.description = row.description;
          recipe.time_recipe = row.time_recipe;
          recipe.popularity = row.popularity;
          recipe.peopleamount = row.peopleamount;
          recipe.imgurl = row.imgurl;
          recipe.title_type = row.title_type;
          recipe.title_origin = row.title_origin;
          recipe.title_difficulty = row.title_difficulty;
          recipe.food_Array = [];
          recipe.food = [];
          recipes.push(recipe);
        }
        recipes.forEach(function(recipe){
          if(recipe.food_Array.indexOf(row.title_food) == -1 && recipe.id_recipe == row.id_recipe){
            recipe.food_Array.push(row.title_food);
            recipe.food.push({title_food: row.title_food, quantity_containfood: row.quantity_containfood})
          }
        });
      });
      var finalRecipes = [];
      ...
      recipes.forEach(function(recipe){
        var correct = true;
        recipe.food.forEach(function(foodRecipe){
          var correct2 = false
          req.query.food.forEach(function(foodUser){
            if(foodUser.tile_food == foodRecipe.title_food && foodUser.quantity_getfood > foodRecipe.quantity_containfood){
              correct2 = true;
            }
          });
