...
var getAll = function(req, res){
  pg.connect(uripg  ,function(err, client, done) {
    if (err) throw err;
    client
      .query("SELECT r.id_recipe, r.title_recipe, d.title_difficulty as difficulty, t.title_type as type, r.time_recipe as time, r.popularity , r.peopleamount as people, o.title_origin as origin, r.description, r.imgurl FROM public.recipe r, public.difficulty d, public.origin o, public.type t WHERE r.id_difficulty = d.id_difficulty AND r.id_origin = o.id_origin AND r.id_type = t.id_type;")
...
var addView = function(req, res){
  pg.connect(uripg  ,function(err, client, done) {
    if (err) throw err;
    client
      .query("UPDATE public.recipe SET popularity = popularity + 1 WHERE id_recipe = $1;"
      , [req.body.id_recipe])
      .on('end', function(result) { done();
        ///
      });
...
var getFavorites = function(req,res){
  ...
  pg.connect(uripg  ,function(err, client, done) {
    if (err) throw err;
    client
      .query("SELECT id_recipe FROM public.favorite WHERE id_user = $1;"
      , [req.query.id_user])
      .on('row', function(row) {
        favorites.push(row.id_recipe);
...
var isFavorite = function(req,res,next){
  pg.connect(uripg  ,function(err, client, done) {
    if (err) throw err;
    client
      .query("SELECT * FROM public.favorite WHERE id_user = $1 AND id_recipe = $2;"
      , [req.query.id_user,req.query.id_recipe])
      .on('end',function(result){ done();
        res.isFavorite = (result.rows.length == 0);
        next();
      });
  });
}

var changeFavorite = function(req,res,next){
  var reqSql;
  if(res.isFavorite){
    reqSql = "INSERT INTO public.favorite (id_user,id_recipe)VALUES($1,$2);";
  }
  else{
      reqSql = "DELETE FROM public.favorite WHERE id_user = $1 AND id_recipe = $2;";
  }
  pg.connect(uripg  ,function(err, client, done) {
    if (err) throw err;
    client
      ...
      , [req.query.id_user,req.query.id_recipe])
      .on('end',function(result){ done();
        res.send({isFavorite:res.isFavorite});
      });
  });
}
