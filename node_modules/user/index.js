...
var addStayConnected = function(req, res, next){
  ...
  pg.connect(uripg  ,function(err, client, done) {
    if (err) throw err;
    client
      .query("UPDATE public.user SET stayconnected = $1, lastconnection = NOW() WHERE mailadress = $2;",[stayconnected, req.body.mailadress])
      .on("end", function(result) { done();
        next();
      ...
      .on("end", function(result) { done();
        if(result.rowCount != 0){
          ...
          let mailOptions = {
              ...
              to: req.query.mailadress, // list of receivers
              subject: 'Nouveau mot de passe', // Subject line
              text: 'Voici votre nouveau mot de passe : ' + newCode, // plain text body
              html: '<b>Voici votre nouveau mot de passe : ' + newCode + '</b>' // html body
          };
